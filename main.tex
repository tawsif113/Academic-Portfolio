...
auditing under a locked evaluation protocol. I have completed a condition-matched
single-run privacy-budget sweep at multiple $\varepsilon$ values; repeated-run
stability analysis is currently in progress.
...
\begin{itemize}
  ...
  \item Established Random Forest, XGBoost, and MLP baselines. At a
  validation-selected threshold of 0.28, the selected baseline MLP achieved
  76.91\% Recall, 23.09\% FNR, and 84.08\% F1 on KDDTest+.
  ...
  remained near chance, creating a floor effect that I treat as an evaluation
  limitation rather than evidence of privacy.
  ...
  \item Completed a condition-matched privacy-budget sweep comparing the
  non-private PyTorch MLP with DP-SGD at actual $\varepsilon=7.9936$, $3.9983$,
  and $1.9990$. Shadow-selected overall MIA ROC-AUC remained approximately 0.50
  across all conditions, and paired 95\% confidence intervals did not support a
  measured overall leakage-reduction claim.

  \item Current work focuses on repeated-run stability analysis for selected
  settings. The $\varepsilon\approx4$ condition is treated only as a candidate
  privacy--utility balance point until repeated runs are complete.
\end{itemize}
